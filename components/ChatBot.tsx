...
const ChatBot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    // FIX: Use Vite Env Var to prevent crash
    const apiKey = import.meta.env.VITE_API_KEY;
    if (!apiKey) {
        setMessages(prev => [...prev, { role: 'model', text: "Error: API Key missing in environment." }]);
        setIsLoading(false);
        return;
    }
    ...
    try {
      ...
      const model = 'gemini-2.0-flash-exp'; // Updated to latest model

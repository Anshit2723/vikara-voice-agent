...
const VideoAnalyzer: React.FC = () => {
  ...
  const startLiveVision = async () => {
    ...
    if (!apiKey) {
        setError('API Key missing (VITE_API_KEY)');
        setIsLoading(false);
        return;
    }
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: 'You are a live vision assistant. Describe what you see and answer questions. Be concise.'
        }
      });
    } catch (err: any) {
      console.error(err);
...
setAnalysis(text || 'No description returned.');
    } catch (err: any) {

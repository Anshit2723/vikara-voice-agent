...
import { Calendar, MessageSquare, Mic, Video, BrainCircuit, Key, ExternalLink, Clock, Trash2, RefreshCcw, User, Sparkles, ChevronRight, XCircle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const analyzeSchedule = async (events: BackendEvent[]) => {
    ...
    try {
      ...
      const prompt = `Based on these calendar events: ${JSON.stringify(events.map(e => ({ title: e.summary, start: e.start.dateTime })))}, provide 3 short, proactive suggestions for the user. Examples: "You have a free block on Friday morning for deep work", "Suggest rescheduling your 2pm as it overlaps with travel", etc. Return exactly 3 bullet points.`;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#080c14] text-slate-100">
      <header className="glass sticky top-0 z-50 px-6 py-4 flex items-center justify-between border-b border-slate-800">
        ...
        <button onClick={handleOpenKey} className="flex items-center gap-2 px-3 py-2 text-slate-500 hover:text-white transition-colors text-xs font-black uppercase tracking-widest">
          <Key size={14} />
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-500 text-sm font-bold uppercase tracking-widest">Real-time Google API Sync</p>
                </div>
                <button 
                ...
                >
                  <RefreshCcw size={20} className={isFetching ? 'animate-spin text-blue-400' : 'text-slate-400'} />
                </button>
              </div>
              ...
              {suggestions.length > 0 && (
                <div className="glass p-8 rounded-[2.5rem] border-blue-500/20 bg-blue-500/5 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="font-black text-xs uppercase tracking-[0.2em] text-blue-400">Gemini Intelligence Suggestions</h4>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {suggestions.map((s, idx) => (
                      <div key={idx} className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800/50 flex items-start gap-3 group hover:border-blue-500/30 transition-all">
              ...
              {liveEvents.length === 0 && !isFetching ? (
                ...
                <div className="grid grid-cols-1 gap-6">
                  {liveEvents.map(event => (
                    <div key={event.id} className="glass group p-8 rounded-[2.5rem] border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-8 hover:bg-slate-900/40 transition-all border-l-[12px] border-l-blue-600">
                      <div className="space-y-4">
                        <div className="space-y-1">
                          <h3 className="text-2xl font-black text-white group-hover:text-blue-400 transition-colors uppercase tracking-tight">{event.summary}</h3>
                          <div className="flex items-center gap-2">
                             <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                             <p className="text-[10px] font-black uppercase text-blue-500 tracking-widest">Live Sync</p>
                          </div>
                        </div>
